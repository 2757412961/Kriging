...
class LSTM_Model(nn.Module):
    def __init__(self, input_size, hidden_size, num_layer, output_size, isbidirectional):
        super(LSTM_Model, self).__init__()
        self.layer1 = nn.LSTM(input_size, hidden_size, num_layer,
                              bidirectional=isbidirectional,
                              bias=True)
        self.bn1 = nn.BatchNorm1d(hidden_size*2, affine=True, track_running_stats=True)
        self.layer2 = nn.Linear(int(hidden_size*2), int(hidden_size*4))
        self.bn2 = nn.BatchNorm1d(hidden_size*4, affine=True, track_running_stats=True)
        self.layer3 = nn.Linear(int(hidden_size*4), int(hidden_size*8))
        self.bn3 = nn.BatchNorm1d(hidden_size*8, affine=True, track_running_stats=True)
        self.layer4 = nn.Linear(int(hidden_size*8), int(hidden_size*4))
        self.bn4 = nn.BatchNorm1d(hidden_size*4, affine=True, track_running_stats=True)
        self.layer5 = nn.Linear(int(hidden_size*4), int(hidden_size*2))
        self.bn5 = nn.BatchNorm1d(hidden_size*2, affine=True, track_running_stats=True)
        self.layer6 = nn.Linear(int(hidden_size*2), int(hidden_size*1))
        self.bn6 = nn.BatchNorm1d(hidden_size*1, affine=True, track_running_stats=True)
        self.layer7 = nn.Linear(int(hidden_size*1), output_size)
        self.bn7 = nn.BatchNorm1d(output_size, affine=True, track_running_stats=True)
        self.dropout = nn.Dropout(0.5)
    ...
    def forward(self, x):  # torch.Size([seq, batch_size, hidden_size])
        ...
        # x = self.bn1(x)
        x = self.layer2(F.relu(self.dropout(x)))  # torch.Size([826, 12])
        # x = self.bn2(x)
        x = self.layer3(F.relu(self.dropout(x)))  # torch.Size([826, 12])
        # x = self.bn3(x)
        x = self.layer4(F.relu(self.dropout(x)))  # torch.Size([826, 12])
        # x = self.bn4(x)
        x = self.layer5(F.relu(self.dropout(x)))  # torch.Size([826, 12])
        # x = self.bn5(x)
        x = self.layer6(F.relu(self.dropout(x)))  # torch.Size([826, 12])
        # x = self.bn6(x)
        x = self.layer7(F.relu(self.dropout(x)))  # torch.Size([826, 12])
        # x = self.bn7(x)
...
def createYst(data, depth, day):
    ...
    # 训练模型
    model, accuracy = train(model, train_X, test_X, train_Y, test_Y, 1000)
...
def GeographyBiLSTM(data, columns, depth, day):
    ...
    # 建立模型
    Yst = createYst(data, depth, day)  # 变异函数 计算Yst
