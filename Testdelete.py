...
def createYs(data):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            se = calSemi(data[i][4], data[j][4])
            X.append([data[i][0], data[i][1], data[j][0], data[j][1],
                      data[i][0] - lon, data[i][1] - lat,
                      data[j][0] - lon, data[j][1] - lat,
                      data[i][3] - day, data[j][3] - day,
                      ds, dt
                      ])
            Y.append(se)
    ...
    # 设置LSTM模型数据类型形状
    train_X = torch.from_numpy(train_X).view(-1, 1, n_input).float()
    test_X = torch.from_numpy(test_X).view(-1, 1, n_input).float()
    train_Y = torch.from_numpy(train_Y).view(-1, 1, 1).float()
    test_Y = torch.from_numpy(test_Y).view(-1, 1, 1).float()

    # 训练模型
    model, accuracy = train(Ys, train_X, test_X, train_Y, test_Y, 800)

    # 输出结果
    def Yst(x1, y1, z1, t1, x2, y2, z2, t2):
        ds = calSDis(x1, y1, x2, y2)
        dt = calTDis(t1, t2)
        X_input = np.array([x1, y1, x2, y2,
                            x1 - lon, y1 - lat,
                            x2 - lon, y2 - lat,
                            t1 - day, t2 - day,
                            ds, dt
                            ])
        input = torch.from_numpy(X_input).view(-1, 1, n_input).float()
        out = model(input)
...
def calMatrixK(data, Yst):
    ...
    for i in range(n):
        templst = []
        for j in range(n):
            yst = Yst(data[i][0], data[i][1], data[i][2], data[i][3],
...
# 计算任意点的向量D
def calMaxtrixD(data, Yst, x, y, z, t):
    D = []
    for row in data:
        yst = Yst(row[0], row[1], row[2], row[3], x, y, z, t)
        D.append([yst])
...
def SpatioTemporalNeuralNetworkKriging(dataS, dataT, data, columns, depth, day):
    ...
    # dataT = transDataStr2Float(dataT) ; drawDataT(dataT)
